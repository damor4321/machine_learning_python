...
print "Percentage of safe loans  : %.3f" % (len(safe_loans_raw) * 100 / (float(len(safe_loans_raw) + len(risky_loans_raw))))
print "Percentage of risky loans : %.3f" % (len(risky_loans_raw) * 100 / (float(len(safe_loans_raw) + len(risky_loans_raw))))
...
false_positives = validation_data[ (validation_data['safe_loans'] == -1) & (validation_data['prediction'] == 1) ]
...
false_negatives = validation_data[ (validation_data['safe_loans'] == 1) & (validation_data['prediction'] == -1) ]
...
total_cost_of_mistakes = len(false_negatives) * 10000 + len(false_positives) * 20000 


# In[62]:

print total_cost_of_mistakes
